...
namespace PluralsightCosmosDb.Demos
{
    public static class StoredProceduresDemo
    {
        ...
        private static async Task Execute_spHelloWorld(DocumentClient client)
        {
            Console.WriteLine();
        ...
        private static async Task Execute_spSetNorthAmerica3(DocumentClient client)
        {
            ...
            try
            {
                ...
                var uri = UriFactory.CreateStoredProcedureUri("mydb", "mystore", "spSetNorthAmerica");
                var options = new RequestOptions { PartitionKey = new PartitionKey("12345") };
                await client.ExecuteStoredProcedureAsync<object>(uri, options, documentDefinition, true);
            }
